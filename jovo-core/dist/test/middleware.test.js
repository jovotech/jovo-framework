...
const Extensible_1 = require("./../src/core/Extensible");
const Middleware_1 = require("./../src/core/Middleware");
class Parent extends Extensible_1.Extensible {
    /**
...
test('test Middleware class initialization', () => {
    const middleware = new Middleware_1.Middleware('name', new Parent());
...
test('test middleware before/after and events', async () => {
    const parent = new Parent();
    parent.actionSet = new src_1.ActionSet(['middleware1'], parent);
    const test = {
        // @ts-ignore
        fn1() {
            // @ts-ignore
