...
test('is empty on initiation', () => {
    const sb = new src_1.SpeechBuilder();
    expect(sb.toString()).toBe('');
});
test('is ssml string with <speak> tags', () => {
    const ssmlString = src_1.SpeechBuilder.toSSML('test');
    expect(ssmlString).toBe('<speak>test</speak>');
});
test('is ssml string without duplicate <speak> tags', () => {
    const ssmlString = src_1.SpeechBuilder.toSSML('<speak>test</speak>');
    expect(ssmlString).toBe('<speak>test</speak>');
});
test('setProsody', () => {
    const speechBuilder = new src_1.SpeechBuilder();
    ...
    const speechBuilder2 = new src_1.SpeechBuilder();
...
test('addText', () => {
    ...
    // test variation
    const speechBuilder3 = new src_1.SpeechBuilder();
    ...
    const speechBuilder4 = new src_1.SpeechBuilder();
    ...
    const speechBuilder5 = new src_1.SpeechBuilder();
    ...
    const speechBuilder6 = new src_1.SpeechBuilder();
...
test('test escapeXML', () => {
    expect(src_1.SpeechBuilder.escapeXml('<')).toBe('&lt;');
    expect(src_1.SpeechBuilder.escapeXml('>')).toBe('&gt;');
    expect(src_1.SpeechBuilder.escapeXml('&')).toBe('&amp;');
    expect(src_1.SpeechBuilder.escapeXml('"')).toBe('&quot;');
    expect(src_1.SpeechBuilder.escapeXml("'")).toBe('&apos;');
...
test('test replace ampersand ', () => {
    expect(src_1.SpeechBuilder.toSSML('Jack & jane <audio src="foo&bar"/>')).toBe('<speak>Jack and jane <audio src="foo&bar"/></speak>');
});
test('test build()', () => {
    const speechBuilder = new src_1.SpeechBuilder();
    expect(speechBuilder.build()).toBe('');
});
//# sourceMappingURL=speechbuilder.test.js.map