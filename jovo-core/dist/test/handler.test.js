...
const Handler_1 = require("../src/plugins/Handler");
...
test('test throw exception on non existing route', (done) => {
    const jovo = {
        ...
        $type: {
            type: src_1.EnumRequestType.INTENT,
        },
    };
    // @ts-ignore
    Handler_1.Handler.applyHandle(jovo, {
...
test('test skip throwing exception on optional request types', async (done) => {
    ...
    // @ts-ignore
    await Handler_1.Handler.applyHandle(jovo, {
        path: 'OPTIONAL_TYPE',
        type: 'OPTIONAL_TYPE',
    });
    done();
...
test('test handleOnPromise on route that returns rejected promise', async () => {
    ...
    try {
        ...
        // @ts-ignore
        await Handler_1.Handler.handleOnPromise(jovo, jovo.$handlers.ON_REQUEST);
    }
    catch (e) {
        expect(e).toEqual(rejectionReason);
    }
...
test('test applyHandle on route with callback', async () => {
    const jovoObj = {
        $handlers: {
            IntentA(jovo, callback) {
                callback();
            },
        },
    };
    // @ts-ignore
    await Handler_1.Handler.applyHandle(jovoObj, {
        path: 'IntentA',
        type: src_1.EnumRequestType.INTENT,
    });
});
...
test('test handleOnPromise on route with callback that immediately throws exception', async () => {
    ...
    try {
        ...
        // @ts-ignore
        await Handler_1.Handler.handleOnPromise(jovoObj, jovoObj.$handlers.ON_REQUEST);
    }
    catch (e) {
        expect(e).toEqual(new Error(errorMessage));
    }
...
test('test handleOnRequest', () => {
    ...
    // @ts-ignore
    Handler_1.Handler.handleOnRequest(jovo);
...
test('test handleOnNewSession (NEW_SESSION=true)', async () => {
    ...
    // @ts-ignore
    await Handler_1.Handler.handleOnNewSession(jovo);
    expect(spy).toBeCalled();
...
test('test handleOnNewSession (NEW_SESSION=false)', () => {
    ...
    const spy = jest.spyOn(jovo.$handlers, 'NEW_SESSION');
    // @ts-ignore
    Handler_1.Handler.handleOnNewSession(jovo);
...
test('test handleOnNewUser (NEW_USER=true)', async () => {
    ...
    // @ts-ignore
    await Handler_1.Handler.handleOnNewUser(jovo);
    expect(spy).toBeCalled();
