...
In a Jovo app, `i18n` means that content is stored in distinct language files (e.g. `en.json` or `en-US.json`) instead of inside [handlers](./handlers.md) or [output classes](./output-classes.md). The app then retrieves the right content depending on the current request's language/locale.
...
Arrays of strings can be used to randomize content for [`message`](https://v4.jovo.tech/docs/output-templates#message) and [`reprompt`](https://v4.jovo.tech/docs/output-templates#reprompt) elements of an output template. Only one of the strings gets randomly selected. This allows for variation in your app's content without making any code modifications.
...
The `$t()` method can also return full objects. This allows for some interesting and flexible ways how to structure your content. For example, `i18n` resources could be structured in the same way as Jovo [output templates](https://v4.jovo.tech/docs/output-templates):
