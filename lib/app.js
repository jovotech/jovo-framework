...
class BaseApp extends EventEmitter {
     ...
     */
    setContext(context) {
        if (!_.isInteger(context.prevLevel)) {
            throw new Error('prevLevel has to be a integer');
        } else if (context.prevLevel < 0) {
            throw new Error('prevLevel has to be bigger than or equal to 0');
        }
        this.config.context = _.merge(this.config.context, context);
    }
