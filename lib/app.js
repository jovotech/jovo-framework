...
class BaseApp extends EventEmitter {
    ...
    constructor(config) {
        ...
        // talk.to.me hackathon version
        if (process.argv.indexOf('--jovo-webhook') > -1) {
            let languageModelDir = process.cwd() + path.sep + 'models';
            if (process.argv.indexOf('--projectDir') > -1 ) {
                languageModelDir = process.argv[process.argv.indexOf('--projectDir') + 1] + path.sep + 'models';
            }
            if (process.argv.indexOf('--disable-jovo-debugger') === -1) {
                this.register('Logging', new JovoDebuggerPlugin({languageModelDir: languageModelDir}));
            }
        }
    ...
    setHandler(handlers) {
        ...
        if (process.argv.indexOf('--language-model-test') > -1 ) {
            let locale;
            const indexOfLanguageModelTestOption = process.argv.indexOf('--language-model-test');

            if (process.argv.length > (indexOfLanguageModelTestOption + 1)) {
                const validLocaleRegex = /^([\w]{2})(-[\w]{2})?$/; // en, en-US, de-DE

                if (validLocaleRegex.test(
                    process.argv[indexOfLanguageModelTestOption + 1].toLowerCase())) {
                    locale = process.argv[indexOfLanguageModelTestOption + 1];
                }
            }
            ...
            if (!fs.existsSync(languageModelDir)) {
                throw new Error(`Can't find your /models folder`);
            }

            if (locale && !fs.existsSync(path.join(languageModelDir, locale + '.json'))) {
                throw new Error(`Can't find locale ${locale} in your /models folder`);
            }

            if (!locale) {
                let files = fs.readdirSync(languageModelDir);

                if (files.length === 0) {
                    throw new Error(`Can't find any locale in your /models folder`);
                }

                locale = files[0].substr(0, files[0].indexOf('.json'));
            }

            let languageModel = require(path.join(languageModelDir, locale + '.json'));
            //
            // LAUNCH
            ...
            for (let intent of languageModel.intents) {
                testLanguageModelHandler[intent.name] = function() {
                    this.ask(intent.name, 'Say the next phrase');
                };
            }
            console.log();
