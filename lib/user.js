...
class User {
     ...
     */
    updateContext() {
        if (_.get(this, 'config.context.prevLevel') > 0) {
    ...
    updateContextPrev() {
        ...
        if (_.get(this, 'config.context.prev.response.speech')) {
        ...
        if (_.get(this, 'config.context.prev.response.reprompt')) {
        ...
        if (_.get(this, 'config.context.prev.response.timestamp')) {
            prevObject.response.timestamp = this.platform.getTimestamp();
        }
        if (_.get(this, 'config.context.prev.response.state')) {
            prevObject.response.state = this.platform.getState();
        }
        if (_.get(this, 'config.context.prev.request.intent')) {
            if (this.platform.getRequestType() === BaseApp.REQUEST_TYPE_ENUM.INTENT) {
                prevObject.request.intent = this.platform.getIntentName();
            } else if (this.platform.getRequestType() === BaseApp.REQUEST_TYPE_ENUM.LAUNCH) {
                prevObject.request.intent = 'LaunchRequest';
            }
        }
        if (_.get(this, 'config.context.prev.request.timestamp')) {
            prevObject.request.timestamp = this.platform.getTimestamp();
        }
        if (_.get(this, 'config.context.prev.request.state')) {
            prevObject.request.state = this.platform.getState();
        }
        if (_.get(this, 'config.context.prev.request.inputs')) {
            if (this.platform.getType() === BaseApp.PLATFORM_ENUM.ALEXA_SKILL) {
                if (this.platform.getRequestType() === BaseApp.REQUEST_TYPE_ENUM.INTENT) {
                    prevObject.request.inputs = this.platform.getInputsObj();
                }
            } else {
                prevObject.request.inputs = this.platform.getInputs();
            }
        }
        ...
        if (this.context.prev.length === _.get(this, 'config.context.prevLevel')) {
