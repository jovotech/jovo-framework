...
class User {
     ...
     */
    getPermissionToken() {
        if (this.platform.getType() === 'AlexaSkill') {
...
class AlexaDeviceAddress {
     ...
     * @param {string} locale
     */
    constructor(permissionToken, deviceId, locale) {
        this.permissionToken = permissionToken;
        this.deviceId = deviceId;

        if (locale === 'en-US') {
            this.apiUrl = 'https://api.amazonalexa.com';
        } else {
            this.apiUrl = 'https://api.eu.amazonalexa.com';
        }
    }
    ...
    getLocation(type) {
        return new Promise((resolve, reject) => {
            ...
            let options = {
                url: this.apiUrl + '/v1/devices/' + this.deviceId + '/settings/'+ type,
                headers: {
                    'Accept': 'application/json',
                },
                auth: {
