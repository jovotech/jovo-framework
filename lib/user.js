...
class User {
    ...
    updateMetaData() {
        ...
        if (_.get(this, 'config.userMetaData.requestHistorySize') > 0) {
            if (!this.metaData.requests) {
                this.metaData.requests = {};
            }
            if (!this.metaData.requests[this.platform.jovo.getHandlerPath()]) {
                let requests = {
                    count: 0,
                    log: [],
                };
                this.metaData.requests[this.platform.jovo.getHandlerPath()] = requests;
            }
