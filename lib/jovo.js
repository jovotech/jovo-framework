...
class Jovo extends Platform {
     ...
     */
    repeat() {
        let speech = this.user().context.prev[0].response.speech;
        let reprompt = this.user().context.prev[0].response.reprompt;
    ...
    loadUserDataOnRequest() {
        return new Promise((resolve, reject) => {
            ...
            return this.user().loadDataFromDb().then(resolve);
        });
    }
