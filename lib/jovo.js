...
class Jovo extends EventEmitter {
    ...
    setConfig(config) {
        ...
        if (typeof config.intentsToSkipUnhandled !== 'undefined') {
            this.intentsToSkipUnhandled = config.intentsToSkipUnhandled;
        }
    ...
    handleStateIntentRequest() {
        ...
        if (!this.handlers[this.getState()][intentToRedirect]) {
            ...
            if (this.handlers[this.getState()][UNHANDLED] && this.intentsToSkipUnhandled.indexOf(intentToRedirect) === -1) {
    ...
    setIntentsToSkipUnhandled(intents) {
        this.intentsToSkipUnhandled = intents;
    }
    ...
    addBespokenAnalytics(apiKey) {
        this.addAnalytics('bespoken_alexa', new Analytics.BespokenAnalytics(apiKey, PLATFORM_ENUM.ALEXA_SKILL));
        this.addAnalytics('bespoken_google', new Analytics.BespokenAnalytics(apiKey, PLATFORM_ENUM.GOOGLE_ACTION));
    }
