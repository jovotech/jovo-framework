...
// Platforms
const AlexaSkill = require('./platforms/alexa/alexaSkill');
...
class Jovo extends Platform {
    ...
    handleElementSelectedRequest() {
        ...
        if (this.config.handlers[BaseApp.REQUEST_TYPE_ENUM.ON_ELEMENT_SELECTED][this.getSelectedElementId()]) {
            elementId = this.getSelectedElementId();
        }

        if (elementId === BaseApp.UNHANDLED &&
            !this.config.handlers[BaseApp.REQUEST_TYPE_ENUM.ON_ELEMENT_SELECTED][BaseApp.UNHANDLED]) {
    ...
    handleAudioPlayerRequest() {
        ...
        if (this.config.handlers['AUDIOPLAYER'][this.getPlatform().audioPlayer().getType()]) {
            this.config.handlers['AUDIOPLAYER'][this.getPlatform().audioPlayer().getType()].call();
        }
    }
     ...
     */
    db(name) {
        if (this.type === BaseApp.TYPE_ENUM.LAMBDA && this.moduleDatabase.databases.file) {
