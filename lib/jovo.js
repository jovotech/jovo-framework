...
class Jovo extends Platform {
    ...
    handleFunction(context, request) {
        ...
        if (_.get(this, 'app.moduleDatabase.databaseInstances.file')) {
            if (_.get(
                    this,
    ...
    handleAudioPlayerRequest() {
        if (!this.config.handlers['AUDIOPLAYER'] &&
            !this.config.handlers['MEDIARESPONSE']) {
        ...
        if (!_.get(
                this.config.handlers,
                `AUDIOPLAYER["${this.getPlatform().audioPlayer().getType()}"]`) &&
            !_.get(
                this.config.handlers,
                `MEDIARESPONSE["${this.getPlatform().audioPlayer().getType()}"]`)) {
        ...
        if (_.get(
            this.config.handlers,
                `MEDIARESPONSE["${this.getPlatform().audioPlayer().getType()}"]`)) {
            this.config.handlers['MEDIARESPONSE'][this.getPlatform().audioPlayer().getType()].apply(this);
        }
    }
    ...
    handleUndefinedRequest() {
        for (let obj of Object.keys(this.config.handlers)) {
            ...
            if (value) {
                if (_.get(this.getRequestObject(), path) === value ) {
                    this.config.handlers[obj].apply(this);
                }
            } else {
                if (_.get(this.getRequestObject(), obj) ) {
                    this.config.handlers[obj].apply(this);
                }
            }
        }
