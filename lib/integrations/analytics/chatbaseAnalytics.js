...
class Chatbase {
     ...
     */
    createChatbaseDataObjectAlexa(app) {
     ...
     * @return {object} data needed to submit the message
     */
    createChatbaseDataObjectGoogleAction(app) {
        let sessionId = '';
        if (app.googleAction().getRequest().constructor.name === 'GoogleActionDialogFlowRequest') {
            ...
            sessionId = app.googleAction().getRequest().originalDetectIntentRequest.payload.conversation.conversationId;
        }
    ...
    sendDataToChatbase(data) {
        ...
        const httpRequest = https.request(options, function(res) {
    ...
    track(app) {
        const data = this.platformType === BaseApp.PLATFORM_ENUM.ALEXA_SKILL ? 
            this.createChatbaseDataObjectAlexa(app) : this.createChatbaseDataObjectGoogleAction(app);
