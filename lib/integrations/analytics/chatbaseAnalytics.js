    ...
    createChatbaseDataAlexa(app) {
        if (app.alexaSkill().getRequest().session) {
            ...
            if (app.alexaSkill().getRequest().request.intent) {
                ...
                var slots = [];
                var intentSlots = app.alexaSkill().getRequest().request.intent.slots;
            ...
            let outputSpeech = app.alexaSkill().getResponse().responseObj.response.outputSpeech;
            ...
            let timestamp = app.alexaSkill().getRequest().request.timestamp;

            const data = {messages: [{
                api_key: this.apiKey,
                type: 'user',
                user_id: app.alexaSkill().getUserId(),
                time_stamp: Date.parse(timestamp),
                platform: 'Alexa',
                intent: intentName,
                message,
                not_handled: app.getHandlerPath().endsWith('Unhandled'),
