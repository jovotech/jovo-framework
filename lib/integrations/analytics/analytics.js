 ...
 */
class BespokenAnalytics {

    /**
     * Constructor
     * @param {string} apiKey Secret Key from Bespoken
     ...
     * @param {string} type Indicates if we are logging request or response (request/response)
     * @param {Object} payload Captured request or response
     ...
     * @param {object} data Wrapped request or response.
     */
     ...
     */
    track(app) {
        const jovoRequest = this.platformType === Jovo.PLATFORM_ENUM.ALEXA_SKILL ?
            app.alexaSkill().getRequest().getRequestObject() :
            app.googleAction().getRequest().getRequestObject();

        const jovoResponse = this.platformType === Jovo.PLATFORM_ENUM.ALEXA_SKILL ?
            app.alexaSkill().getResponse().getResponseObject() :
            app.googleAction().getResponse().getResponseObject();
...
module.exports.BespokenAnalytics = BespokenAnalytics;
