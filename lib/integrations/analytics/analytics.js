        ...
        if (this.isEnabled()) {
            for (let i = 0; i < Object.keys(this.services).length; i++) {
                ...
                if (this.intentsToSkip.length > 0 &&
                    this.intentsToSkip[app.getIntentName()]) {
                    continue;
                }

                if (this.intentsToTrack.length > 0 &&
                    !this.intentsToTrack[app.getIntentName()]) {
                    continue;
                }
...
class VoiceLabsAlexaAnalytics {
    ...
    track(app) {
        this.voiceLabsAlexa.track(
            ...
            app.getIntentName(),
            this.trackSlots ? app.alexaSkill().getSlots() : null,
...
class VoiceLabsGoogleActionAnalytics {
     ...
     */
    track(app) {
        let request = app.googleAction().getOriginalRequest() ?
            app.googleAction().getOriginalRequest() : null;
...
class DashbotGoogleActionAnalytics {
     ...
     */
    track(app) {
        this.dashbot.logIncoming(app.googleAction().request);
        this.dashbot.logOutgoing(app.googleAction().request,
            app.googleAction().getResponseObject());
    };
}
...
class DashbotAlexaAnalytics {
     ...
     */
    track(app) {
        this.dashbot.logIncoming(app.alexaSkill().request);
        this.dashbot.logOutgoing(app.alexaSkill().request,
            app.alexaSkill().getResponseObject());
    };
}
