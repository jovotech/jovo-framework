...
class FilePersistence {
    ...
    load(key, callback) {
        ...
        fs.readFile(this.filename, 'utf8', function(err, data) {
            ...
            if (!fileObj[mainKey]) {
                callback(createMainKeyNotFoundError(), null);
                return;
            }

            if (!fileObj[mainKey][key]) {
                callback(createDataKeyNotFoundError(), null);
                return;
            }
 ...
 * @return {Error}
 */
function createDataKeyNotFoundError() {
 ...
 * @return {Error}
 */
function createMainKeyNotFoundError() {
