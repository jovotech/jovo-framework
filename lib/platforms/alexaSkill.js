...
class AlexaSkill {
     ...
     */
    getEndReason() {
        console.log(this.getRequestType());
    ...
    getInputsObj() {
        ...
        for (let i = 0; i < slotNames.length; i++) {
            ...
            inputs[key] = {
                name:
                this.request.request.intent.slots[key].name,
                value:
                this.request.request.intent.slots[key].value,
                confirmationStatus:
                this.request.request.intent.slots[key].confirmationStatus,
            };
        }
        return inputs;
    }
     ...
     */
    getDialogState() {
        if (this.request.request.dialogState === DIALOGSTATE_ENUM.STARTED) {
            return jovo.DIALOGSTATE_ENUM.STARTED;
        }
        if (this.request.request.dialogState === DIALOGSTATE_ENUM.IN_PROGRESS) {
            return jovo.DIALOGSTATE_ENUM.IN_PROGRESS;
        }
        if (this.request.request.dialogState === DIALOGSTATE_ENUM.COMPLETED) {
