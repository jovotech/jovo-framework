...
class GoogleActionResponseV2 {
     ...
     * @return {*}
     */
    getContextOut(name) {                                       // TODO name still makes sense?
            ...
            if (speechText) {
                if (speechText.constructor === Array) {
                    let filteredSpeechText = speechText.filter((text) => {
                        return this.responseObj.fulfillmentText === toSSML(text) && this.responseObj.payload.google.richResponse
     ...
     * @return {boolean}
     */
    isAsk(speechText, repromptText) {                                                                                // todo done
        try {
            ...
            if (repromptText) {
                if (repromptText.constructor === Array) {
                    let filteredSpeechText = speechText.filter((text) => {
                        return this.responseObj.payload.google.noInputPrompts[0].ssml === toSSML(text);
                    });
                    if (filteredSpeechText.length === 0) {
                        return false;
                    }
     ...
     * @return {boolean}
     */
    isPlay(audioUrl, fallbackText) {                                                                                // todo done
        let speech = `<speak><audio src="${audioUrl}">${fallbackText}</audio></speak>`;
