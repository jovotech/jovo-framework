...
class AudioPlayer {
    ...
    play(url, token, playBehavior) {
        ...
        this.response.audioPlayerPlay(playBehavior, this.createAudioItem(url, token, playBehavior));
     ...
     * @return {*}
     */
    createAudioItem(url, token, playBehavior) {
        let stream = {
            stream: {
                token: token,
                url: url,
        ...
        if (this.expectedPreviousToken) {
            ...
            if (playBehavior === 'ENQUEUE') {
                throw new Error('A Play directive with the ENQUEUE play behavior must have an expected token');
            }
        }
