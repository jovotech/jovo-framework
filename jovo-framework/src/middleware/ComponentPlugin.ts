...
class ComponentPlugin extends Extensible {
	...
	static setActiveComponentPlugins(jovo: Jovo) {
		const componentSessionStack: Array<[string, ComponentSessionData]> = jovo.$session.$data[SessionConstants.COMPONENT];
	...
	static saveComponentSessionData(handleRequest: HandleRequest): void {
		...
		if (!componentSessionStack || componentSessionStack.length < 1) {
			...
			const activeComponentObject: Component = handleRequest.jovo.$components[latestComponentFromSessionStack[0]];
	...
	static loadLatestComponentSessionData(jovo: Jovo): void {
		if (!jovo.$session.$data[SessionConstants.COMPONENT] || jovo.$session.$data[SessionConstants.COMPONENT].length < 1) {
			return;
		} else {
			const componentSessionArray: Array<[string, ComponentSessionData]> = jovo.$session.$data[SessionConstants.COMPONENT];
			const activeComponentData: [string, ComponentSessionData] = componentSessionArray[componentSessionArray.length - 1];
