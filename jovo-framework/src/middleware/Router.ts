...
export class Router implements Plugin {
    ...
    static mapIntentName(
        ...
        if (component) {
            ...
            if (componentIntentMap && componentIntentMap[intentName]) {
                Log.verbose(
                    `Mapping intent from ${intentName} to ${
                        componentIntentMap[intentName]
                        }`
                );
        ...
        if (appConfig.intentMap && appConfig.intentMap[intentName]) {
            Log.verbose(
                `Mapping intent from ${intentName} to ${
                    appConfig.intentMap[intentName]
                    }`
            );
	...
	async router(handleRequest: HandleRequest) {
		...
		if (handleRequest.jovo.$type.type && handleRequest.jovo.$type.subType) {
			route.path = `${handleRequest.jovo.$type.type}["${
