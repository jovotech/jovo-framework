...
export class JovoUser implements Plugin {
	...
	install(app: BaseApp): void {
		...
		User.prototype.loadData = async function() {
			...
			return loadDb(
				new HandleRequest(this.jovo.$app, this.jovo.$host, this.jovo),
				true
			);
		};
	...
	loadDb = async (handleRequest: HandleRequest, force = false) => {
		...
		if (
			handleRequest.app.$db.needsWriteFileAccess &&
		...
		handleRequest.jovo.$user.$data = _get(
			data,
			`${this.config.columnName}.data`,
			{}
		);

		if (this.config.metaData && this.config.metaData.enabled) {
			handleRequest.jovo.$user.$metaData = _get(
				data,
				`${this.config.columnName}.metaData`,
				{
		...
		if (this.config.context && this.config.context.enabled) {
			handleRequest.jovo.$user.$context = _get(
				data,
				`${this.config.columnName}.context`,
				{}
			);
		}
		if (this.config.sessionData && this.config.sessionData.enabled) {
			...
			const timeout = this.config.sessionData.timeout || 300000;

			let sessionData = {};
			if (serializedSessionData.lastUpdatedAt) {
				const expirationTime =
					new Date(serializedSessionData.lastUpdatedAt).getTime() + timeout;
		...
		Log.verbose();
		Log.yellow().debug(
			JSON.stringify(handleRequest.jovo.$user.$context, null, '\t')
		);
		Log.yellow().debug('this.$user.$data');
		Log.yellow().debug(
			JSON.stringify(handleRequest.jovo.$user.$data, null, '\t')
		);
		Log.yellow().debug('this.$user.$metaData');
		Log.yellow().debug(
			JSON.stringify(handleRequest.jovo.$user.$metaData, null, '\t')
		);
		Log.yellow().debug('this.$user.$sessionData');
		Log.yellow().debug(
			JSON.stringify(handleRequest.jovo.$user.$sessionData, null, '\t')
		);
	};
	...
	private updatePrevResponseState(
		...
		if (
			...
			prevObject.response!.state = handleRequest.jovo!.$session.$data[
