...
describe('test install()', () => {
    ...
    test('should set saveDb function to be executed with user.save', () => {
        jovoUser.install(app);
        const middleware = app.middleware('user.save');
...
describe('test loadDb() and saveDb()', () => {
    ...
    beforeEach(() => {
        ...
        }; // hack so we don't have to implement the full BaseApp class, but just the parts we need
        const host = {
        ...
        mockHandleRequest = new jovo_core_1.HandleRequest(app, host, jovo);
    });
...
describe('test updateMetaData', () => {
    ...
    beforeEach(() => {
        ...
        jovoUser = new src_1.JovoUser(config);
        const app = {}; // hack so we don't have to implement the full BaseApp class, but just the parts we need
        const host = {};
        const jovo = {
            $request: {},
            $user: {
...
describe('test updateContextData', () => {
    ...
    test('should set response.state', () => {
        jovoUser.config.context.prev.response.state = true;
        mockHandleRequest.jovo.$session.$data[jovo_core_1.SessionConstants.STATE] = 'test';
        jovoUser['updateContextData'](mockHandleRequest); // tslint:disable-line:no-string-literal
    ...
    test('should set request.state', () => {
        ...
        mockHandleRequest.jovo.$requestSessionAttributes[jovo_core_1.SessionConstants.STATE] = 'test';
        jovoUser['updateContextData'](mockHandleRequest); // tslint:disable-line:no-string-literal
    ...
    test(`should set request.intent to intent because it's an IntentRequest`, () => {
        ...
        mockHandleRequest.jovo.$type.type = jovo_core_1.EnumRequestType.INTENT;
...
//# sourceMappingURL=JovoUser.test.js.map