...
test('test basics', () => {
    ...
    expect(app.$plugins.get('BasicLogging').config.logging).toBe(true);
    app.config.plugin.BasicLogging.logging = false;
    expect(app.config.plugin.BasicLogging.logging).toBe(false);
    expect(app.$plugins.get('BasicLogging').config.logging).toBe(false);
});
test('test logging', () => {
    app = new App_1.App({
        logging: true,
    ...
    expect(app.config.plugin.BasicLogging.requestObjects).toEqual(expect.arrayContaining(['a', 'b']));
    expect(app.config.plugin.BasicLogging.responseObjects).toEqual(expect.arrayContaining(['c', 'd']));
    expect(app.config.plugin.BasicLogging.request).toBe(true);
    expect(app.config.plugin.BasicLogging.response).toBe(false);
});
test('test user', () => {
...
test('test intentMap', () => {
    ...
    expect(app.config.plugin.Router.intentMap).toEqual(expect.objectContaining({ IntentA: 'IntentB' }));
    expect(app.config.plugin.Router.intentsToSkipUnhandled).toEqual(expect.arrayContaining(['IntentX', 'IntentY']));
});
test('test intentMap', () => {
    app = new App_1.App({
        intentMap: {
...
test('test i18n', () => {
    ...
    expect(app.config.plugin.I18Next.resources).toEqual(expect.objectContaining({
        'en-US': {
            translation: {
                WELCOME: 'Hello World',
            },
