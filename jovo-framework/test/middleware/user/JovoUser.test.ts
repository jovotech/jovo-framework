...
describe('test loadDb() and saveDb()', () => {
    ...
    describe('test loadDb()', () => {
        ...
        test('should resolve promise because implicitSave & force are false', async () => {
            ...
            await expect(
                jovoUser.loadDb(mockHandleRequest, force),
        ...
        test('should throw error because userId for the request was undefined', async () => {
            expect.assertions(1);
            mockHandleRequest.jovo!.$user.getId = jest
                .fn()
        ...
        test('should load metaData and parse it to $user.$metaData', async () => {
            ...
            expect(metaData).toEqual(
                mockData[ `${jovoUser.config.columnName}.metaData` ],
            );
        });
        ...
        test('should load context and parse it to $user.context', async () => {
            ...
            expect(context).toEqual(
                mockData[ `${jovoUser.config.columnName}.context` ],
            );
        });
    ...
    describe('test saveDb()', () => {
        ...
        test('should resolve promise because implicitSave & force are false', async () => {
            ...
            await expect(
                jovoUser.saveDb(mockHandleRequest, force),
            ).resolves.toBeUndefined();
        });
...
describe('test updateMetaData', () => {
    ...
    test('shouldn\'t update sessionsCount because it\'s turned off in the config', () => {
        ...
        expect(
            mockHandleRequest.jovo!.$user.$metaData.sessionsCount,
        ).toBeUndefined();
    });
    ...
    test('should not let request.log exceed the requestHistorySize', () => {
        ...
        expect(
            mockHandleRequest.jovo!.$user.$metaData.requests!.xyz.log.length,
        ).toBe(2);
    });
    ...
    test('should add new object for new device id', () => {
        ...
        mockHandleRequest.jovo!.hasScreenInterface = jest
            .fn()
            .mockReturnValue(false);
        mockHandleRequest.jovo!.hasVideoInterface = jest
            .fn()
            .mockReturnValue(false);
...
describe('test updateContextData', () => {
    ...
    test('response should be undefined as all of its options are set to false', () => {
        ...
        expect(
            mockHandleRequest.jovo!.$user.$context.prev![ 0 ].response,
        ).toBeUndefined();
    });
    ...
    test('should set response speech to $output.tell', () => {
        ...
        expect(
            mockHandleRequest.jovo!.$user.$context.prev![ 0 ].response!.speech,
        ).toBe('xyz');
    });
    ...
    test('should set response.reprompt', () => {
        ...
        expect(
            mockHandleRequest.jovo!.$user.$context.prev![ 0 ].response!.reprompt,
        ).toBe('reprompt');
    });
    ...
    test('should set response.state', () => {
        ...
        expect(
            mockHandleRequest.jovo!.$user.$context.prev![ 0 ].response!.state,
        ).toBe('test');
    });
    ...
    test('should set response.output', () => {
        ...
        expect(
            mockHandleRequest.jovo!.$user.$context.prev![ 0 ].response!.output,
        ).toBe('test');
    });
    ...
    test('should set request.timestamp', () => {
        jovoUser.config.context!.prev!.request!.timestamp = true;
        mockHandleRequest.jovo!.$request!.getTimestamp = jest
            .fn()
        ...
        expect(
            mockHandleRequest.jovo!.$user.$context.prev![ 0 ].request!.timestamp,
        ).toBe('test');
    });
    ...
    test('should set request.state', () => {
        ...
        mockHandleRequest.jovo!.$requestSessionAttributes[ SessionConstants.STATE ] =
            'test';

        jovoUser[ 'updateContextData' ](mockHandleRequest); // tslint:disable-line:no-string-literal

        expect(mockHandleRequest.jovo!.$user.$context.prev![ 0 ].request!.state).toBe(
            'test',
        );
    });
    ...
    test('shouldn\'t set request.state as it\'s turned off in the config', () => {
        ...
        expect(
            mockHandleRequest.jovo!.$user.$context.prev![ 0 ].request!.state,
        ).toBeUndefined();
    });
    ...
    test('should set request.inputs', () => {
        ...
        expect(
            mockHandleRequest.jovo!.$user.$context.prev![ 0 ].request!.inputs,
        ).toEqual({
            key: 'value',
        });
    });
    ...
    test('should set request.intent to intent because it\'s an IntentRequest', () => {
        ...
        expect(
            mockHandleRequest.jovo!.$user.$context.prev![ 0 ].request!.intent,
        ).toBe('test');
    });
