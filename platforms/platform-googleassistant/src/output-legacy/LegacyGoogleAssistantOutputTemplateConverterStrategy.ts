...
import {
  ...
  CAROUSEL_MIN_SIZE,
  LegacySuggestion,
  LEGACY_SUGGESTION_TITLE_MAX_LENGTH,
  LEGACY_SUGGESTIONS_MAX_SIZE,
} from './index';
...
export class LegacyGoogleAssistantOutputTemplateConverterStrategy extends SingleResponseOutputTemplateConverterStrategy<
  GoogleAssistantLegacyResponse,
  OutputTemplateConverterStrategyConfig
> {
  platformName = 'googleAssistant' as const;
  responseClass = GoogleAssistantLegacyResponse;
  ...
  protected sanitizeQuickReplies(
    ...
    path: string,
    maxSize = LEGACY_SUGGESTIONS_MAX_SIZE,
  ...
  toResponse(output: NormalizedOutputTemplate): GoogleAssistantLegacyResponse {
    const response: GoogleAssistantLegacyResponse = this.normalizeResponse({
      richResponse: {
        items: [],
      },
    });
  ...
  fromResponse(response: GoogleAssistantLegacyResponse): NormalizedOutputTemplate {
  ...
  convertQuickReplyToSuggestion(quickReply: QuickReplyValue): LegacySuggestion {
