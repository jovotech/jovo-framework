...
export interface GetContextDialogflow extends GetContext {
...
export class GetHook extends PluginHook<GetEvents> {
  ...
  async get(): Promise<void> {
    ...
    const getTask: Task = new Task(
      `Getting Dialogflow Agent files for ${printHighlight(this.$context.projectId!)}`,
      async () => {
        ...
        if (keyFilePath) {
          ...
          try {
            const response = await axios({
              method: 'POST',
              url: `https://dialogflow.googleapis.com/v2beta1/projects/${this.$context.projectId}/agent:export`, // eslint-disable-line
              headers: {
            ...
            const zip: AdmZip = new AdmZip(
              Buffer.from(response.data.response.agentContent, 'base64'),
            );
