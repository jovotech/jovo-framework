...
export class Apl {
    ...
    addDocumentDirective(documentDirective: any) { // tslint:disable-line
        const doc = {
    ...
    addCommands(token: string, commands: any[]) { // tslint:disable-line
        ...
        const existingExecuteCommands = _get(this.alexaSkill.$output, 'Alexa.Apl', []).filter((directive: any) => { // tslint:disable-line
...
export class AplPlugin implements Plugin {
    ...
    output(alexaSkill: AlexaSkill) {
        ...
        if ((alexaSkill.$request! as AlexaRequest).hasAPLInterface() || (alexaSkill.$request! as AlexaRequest).hasAPLTInterface()) {
