...
describe(`test internal 'nlu'`, () => {
    ...
    app.setHandler({
      NameState: {
        MyNameIsIntent() {
          ...
          expect((this.$inputs.name as AlexaInput).alexaSkill.name).toEqual('name');
          expect((this.$inputs.name as AlexaInput).alexaSkill.value!.toLowerCase()).toEqual('jeff');
          expect((this.$inputs.name as AlexaInput).alexaSkill.confirmationStatus).toEqual('NONE');
          expect((this.$inputs.name as AlexaInput).alexaSkill.source).toEqual('USER');
          done();
        },
      },
    });
  ...
  test('test intent with slot resolution', async (done) => {
    app.setHandler({
      NameState: {
        MyNameIsIntent() {
          ...
          expect(
            _get(
              (this.$inputs.name as AlexaInput).alexaSkill,
    ...
    const request: JovoRequest = await t.requestBuilder.rawRequestByKey(
      'IntentRequestWithSlotResolution',
    );
    app.handle(ExpressJS.dummyRequest(request));
  });
});
