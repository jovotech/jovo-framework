...
test('test this.$alexaSkill.showStandardCard', async (done) => {
  ...
  app.on('response', (handleRequest: HandleRequest) => {
    ...
    expect(_get(response, 'response.card.title')).toEqual('title');
    expect(_get(response, 'response.card.text')).toEqual('content');
    expect(_get(response, 'response.card.image.smallImageUrl')).toEqual(
      'https://url.to/image_small.jpg',
    );
    expect(_get(response, 'response.card.image.largeImageUrl')).toEqual(
      'https://url.to/image_large.jpg',
    );
    done();
  });
});
...
test('test this.$alexaSkill.showAskForAddressCard', async (done) => {
  ...
  app.on('response', (handleRequest: HandleRequest) => {
    ...
    expect(_get(response, 'response.card.permissions[0]')).toEqual(
...
test('test this.$alexaSkill.showAskForContactPermissionCard', async (done) => {
  ...
  app.on('response', (handleRequest: HandleRequest) => {
    ...
    expect(_get(response, 'response.card.permissions[2]')).toEqual(
