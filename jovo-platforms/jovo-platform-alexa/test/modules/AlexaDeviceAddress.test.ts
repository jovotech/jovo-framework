...
test('test 403 NO_USER_PERMISSION', async () => {
    ...
    const result = AlexaDeviceAddress.deviceAddressApi(AlexaDeviceAddress.ADDRESS,
        'apiEndPoint',
        'permissionToken',
        'deviceId');
    expect(
...
test('test ADDRESS', async () => {
    const addressData = { addressLine1: 'addressLine1',
...
test('test COUNTRY_AND_POSTAL_CODE', async () => {
    ...
    const result = AlexaDeviceAddress.deviceAddressApi(AlexaDeviceAddress.COUNTRY_AND_POSTAL_CODE,
        'apiEndPoint',
        'permissionToken',
        'deviceId');
...
test('test invalid property', async () => {
    expect(
        AlexaDeviceAddress.deviceAddressApi('propertyABC',
            'apiEndPoint',
            'permissionToken',
            'deviceId'))
