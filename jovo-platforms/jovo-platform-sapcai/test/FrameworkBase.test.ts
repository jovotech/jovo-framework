...
beforeEach(() => {
	app = new App();
	const sapcai = new SapCai();
	app.use(sapcai);
	t = sapcai.makeTestSuite();
});
...
describe('test request types', () => {
	...
	test('test intent', async done => {
		...
		const request: JovoRequest = await t.requestBuilder.intent(
			'HelloWorldIntent',
			{}
		);
...
describe('test tell', () => {
	test('tell plain text', async done => {
		...
		const launchRequest: JovoRequest = await t.requestBuilder.launch();
		//const launchRequest: JovoRequest = await t.requestBuilder.intent('HelloWorldIntent', {});
		app.handle(ExpressJS.dummyRequest(launchRequest));
...
describe('test $inputs', () => {
	test('test getInput, $inputs', async done => {
		...
		const intentRequest: JovoRequest = await t.requestBuilder.intent(
			'HelloWorldIntent',
			{
				name: {
					raw: 'Joe',
...
describe('test toIntent', () => {
	...
	test('with async method in called method', async done => {
		...
		app.on('response', (handleRequest: HandleRequest) => {
			expect(
				handleRequest.jovo!.$response!.isTell('to intent after delay')
			).toBe(true);
			done();
		});
	});
	...
	test('test from ON_REQUEST with skipping LAUNCH', async done => {
		...
		app.on('response', (handleRequest: HandleRequest) => {
			expect(handleRequest.jovo!.$response!.isTell('Hello toIntent')).toBe(
				true
			);
			done();
		});
...
describe('test handleOnNewSession', () => {
	...
	// If you don't reset the memory from SAP CAI it will be passed to the new intent
...
describe('test app config', () => {
	...
	test('test keepSessionDataOnSessionEnded (true) ', async done => {
		...
		app.on('response', (handleRequest: HandleRequest) => {
			expect(handleRequest.jovo!.$response!.hasSessionAttribute('foo')).toBe(
				true
			);
			done();
		});
