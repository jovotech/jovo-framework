...
const intentsOptionRequest = require('../../../lib/platforms/googleaction/request/samples/googleActionDialogflow/v1/intentsOptionRequest');


// workaround
response.json = function(json) {};

describe('intent options request', function() {
    it('should return correct option key', function(done) {
        let app = new App();

        let request = RequestBuilderGoogleActionDialogFlow
            .request(intentsOptionRequest);

        app.handleRequest(request.buildHttpRequest(), response, {
            'ON_ELEMENT_SELECTED': function() {
                expect(this.getSelectedElementId())
                    .to.be.equal('Listitem1key');
                done();
            },
        });
    });

    it('should jump unhandled handler ', function(done) {
        let app = new App();
    ...
    it('should return error message if no handler found ', function(done) {
        let app = new App();
        ...
        app.handleRequest(request.buildHttpRequest(), response, {
            ...
            assert.ok(error.message === 'Could not find the route "ON_ELEMENT_SELECTED.Listitem1key" in your handler function.');
            done();
        });
    });
});
