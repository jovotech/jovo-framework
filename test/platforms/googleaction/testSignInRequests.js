...
const signInOkRequest = require('../../../lib/platforms/googleaction/request/samples/googleActionDialogflow/v1/signInOkRequest');
const signInCancelledRequest = require('../../../lib/platforms/googleaction/request/samples/googleActionDialogflow/v1/signInCancelledRequest');
const signInErrorRequest = require('../../../lib/platforms/googleaction/request/samples/googleActionDialogflow/v1/signInErrorRequest');


// workaround
response.json = function(json) {};

describe('sign in requests', function() {
    it('should return correct access token after sign in request', function(done) {
        let app = new App();

        let request = RequestBuilderGoogleActionDialogFlow
            .request(signInOkRequest);

        app.handleRequest(request.buildHttpRequest(), response, {
            'ON_SIGN_IN': function() {
                expect(this.googleAction().getSignInStatus())
                    .to.be.equal('OK');

                expect(this.getAccessToken())
                    .to.be.equal('ACCESS_TOKEN');
                done();
            },
        });
    });
    it('should return cancelled status after sign in request', function(done) {
        let app = new App();

        let request = RequestBuilderGoogleActionDialogFlow
            .request(signInCancelledRequest);
    ...
    it('should return error status after sign in request', function(done) {
        let app = new App();

        let request = RequestBuilderGoogleActionDialogFlow
            .request(signInErrorRequest);
