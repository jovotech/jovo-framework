...
export class AmazonPollyTts implements Plugin {
    ...
    uninstall(parent?: Extensible): void {
    }

    async tts(jovo: Jovo) {
        console.log('[AmazonPollyTTS] ( $tts )');

        await this.applyPolly(jovo, 'tell.speech');
        await this.applyPolly(jovo, 'ask.speech');
        await this.applyPolly(jovo, 'ask.reprompt');
    }

    private async applyPolly(jovo: Jovo, path: string) {
        ...
        if (text.includes('<audio')) {
            ...
            for (const part of parts) {
                if (part.startsWith('<audio')) {
                    ...
                    const pollyResult = await this.textToSpeech(
                        SpeechBuilder.toSSML(part),
                    );
                    if (pollyResult) {
    ...
    private getAudioTagFromPollyResult(
        pollyResult: SynthesizeSpeechOutput,
    ): string {
