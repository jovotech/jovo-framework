...
const _set = require("lodash.set"); // tslint:disable-line:no-implicit-dependencies
const DatastoreDb_1 = require("../src/DatastoreDb");
describe('test install()', () => {
    test('should assign new Datastore to datastore property', () => {
        const datastoreDb = new DatastoreDb_1.DatastoreDb();
        const app = new jovo_core_1.BaseApp();
        datastoreDb.install(app);
        expect(datastoreDb.datastore).toBeInstanceOf(datastore_1.Datastore);
    });
    describe('test install() setting app.$db', () => {
        test('test should set app.$db to be DatastoreDb if no default db was set in config', () => {
            ...
            const app = new jovo_core_1.BaseApp();
            datastoreDb.install(app);
            expect(app.$db).toBeInstanceOf(DatastoreDb_1.DatastoreDb);
        });
        test('test app.$db should not be an instance of DatastoreDb if default db set in config is not DatastoreDb', () => {
            ...
            datastoreDb.install(app);
            expect(app.$db).not.toBeInstanceOf(DatastoreDb_1.DatastoreDb);
        });
...
describe('test errorHandling()', () => {
    let datastoreDb;
    beforeEach(() => {
        datastoreDb = new DatastoreDb_1.DatastoreDb();
    });
...
//# sourceMappingURL=DatastoreDb.test.js.map