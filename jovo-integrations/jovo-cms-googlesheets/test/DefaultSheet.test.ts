...
describe('DefaultSheet.install()', () => {
	...
	test('should register middleware on retrieve', () => {
		...
		fn = googleSheetsCMS
			.middleware('retrieve')!
			.fns.find(i => i.name === 'bound retrieve');
...
describe('DefaultSheet.parse()', () => {
	test('should throw error if entity is not set', () => {
		...
		expect(() => defaultSheet.parse(handleRequest, [])).toThrow(
...
describe('DefaultSheet.parsePublicToPrivate()', () => {
	test('should throw Jovo Error with empty feed', () => {
		const defaultSheet = new DefaultSheet();
		expect(() => defaultSheet['parsePublicToPrivate']({ feed: {} })).toThrow( // tslint:disable-line:no-string-literal
	...
	test('with valid values', () => {
		const defaultSheet = new DefaultSheet();
		expect(defaultSheet['parsePublicToPrivate'](feed)).toStrictEqual( // tslint:disable-line:no-string-literal
			sheetValues
		);
	});
});
...
describe('DefaultSheet.retrieve()', () => {
	test('should reject Promise if no parent is set', async () => {
		const defaultSheet = new DefaultSheet();
		await expect(defaultSheet.retrieve(handleRequest)).rejects.toMatch(
			'No cms initialized.'
		);
	});
