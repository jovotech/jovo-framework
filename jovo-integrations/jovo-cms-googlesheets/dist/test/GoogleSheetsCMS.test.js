...
describe('GoogleSheetsCMS.constructor()', () => {
    ...
    test('with config', () => {
        const googleSheetsCMS = new src_1.GoogleSheetsCMS({ credentialsFile: '../test' });
        expect(googleSheetsCMS.config.credentialsFile).toMatch('../test');
    });
    test('should register "this" in ActionSet on "retrieve"', () => {
        const googleSheetsCMS = new src_1.GoogleSheetsCMS();
        expect(googleSheetsCMS.actionSet.get('retrieve').parent).toStrictEqual(googleSheetsCMS);
    });
});
...
//# sourceMappingURL=GoogleSheetsCMS.test.js.map