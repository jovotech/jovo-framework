...
for (const p of [new jovo_platform_alexa_1.Alexa(), new jovo_platform_googleassistant_1.GoogleAssistant()]) {
    ...
    describe('test request types', () => {
        test('test launch', async (done) => {
            ...
            app.on('response', (handleRequest) => {
                expect(handleRequest.jovo.$type.type).toBe(jovo_framework_1.EnumRequestType.LAUNCH);
                done();
            });
        });
        test('test intent', async (done) => {
            ...
            app.on('response', (handleRequest) => {
                expect(handleRequest.jovo.$type.type).toBe(jovo_framework_1.EnumRequestType.INTENT);
                done();
            });
        });
        test('test end', async (done) => {
            ...
            app.handle(jovo_framework_1.ExpressJS.dummyRequest(request));
            app.on('response', (handleRequest) => {
                expect(handleRequest.jovo.$type.type).toBe(jovo_framework_1.EnumRequestType.END);
                done();
            });
        });
    ...
    describe('test handleOnNewSession', () => {
        ...
        test('NEW_SESSION but request with old session', async (done) => {
            ...
            const intentRequest = await t.requestBuilder.intent('IntentA');
            app.handle(jovo_framework_1.ExpressJS.dummyRequest(intentRequest.setNewSession(false)));
            app.on('response', (handleRequest) => {
                done();
            });
        });
    ...
    describe('test app listener', () => {
        ...
        test('test onFail', async (done) => {
            expect.assertions(1);
            // @ts-ignore
            process.env.JOVO_LOG_LEVEL = jovo_framework_1.LogLevel.NONE;
            app.setHandler({
                LAUNCH() {
    ...
    test('test intentMap with predefined handler path', async (done) => {
        ...
        const request = await t.requestBuilder.intent('StopIntent');
        app.handle(jovo_framework_1.ExpressJS.dummyRequest(request));
        app.on('response', (handleRequest) => {
            expect(handleRequest.jovo.$response.isTell('Hello!')).toBeTruthy();
            done();
